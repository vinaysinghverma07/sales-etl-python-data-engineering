...
def transform_data(df: pd.DataFrame, logger) -> pd.DataFrame:
    ...
    "pd.DataFrame" after a sign "->" is known as  return annotation or a return type hint. (Shared below)
    And "pd.DataFrame" after "df:" is known as "Annotation" or "parameter type hint" or what should be
        datatype/expected data for that parameter
    ...
    # # Load configuration (used only for logging setup)
    # config = load_config()
    #
    # # Setup logger using config values
    ...
    logger.info("Missing values in 'quantity' filled with 1")
    ...
    logger.info(f"Removed {removed_rows} duplicate rows based on order id, which is 4")
